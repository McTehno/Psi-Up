...
import { sendModuleAssistantMessage } from '../../../services/module-assistant-service'
...
function ModuleAssistantBox({
  ...
  variant = 'desktop',
  className = '',
}: ModuleAssistantBoxProps) {
  const messagesContainerRef = useRef<HTMLDivElement | null>(null)
  const latestQuestionRef = useRef<HTMLDivElement | null>(null)

  const storageKey = useMemo(() => `module-assistant:${moduleId}`, [moduleId])
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!question || isLoading || !moduleId) {
      return
    }
    ...
    try {
      const response = await sendModuleAssistantMessage({
  ...
  return (
    <CollapsibleChatPanel variant={variant} className={className} title="AI pomočnik">
      <div
        className={
          variant === 'mobile'
            ? 'flex h-[min(500px,calc(100vh-190px))] min-h-[320px] flex-col'
      ...
      >
          ...
          {state.messages.length === 0 ? (
            <div>
              <p className="text-sm leading-6 text-[#5f6652]">
                Tukaj se bo prikazal pogovor s pomočnikom. Odgovarjal bo samo na
                podlagi podatkov tega modula in njegovih učnih enot.
              </p>
        ...
        <form onSubmit={handleSubmit} className="mt-4 shrink-0">
          ...
          <div className="flex items-end gap-2 rounded-[1.35rem] border border-[#b8a486]/80 bg-white/45 p-1.5 transition focus-within:border-[#31583b] focus-within:bg-white/65 focus-within:ring-4 focus-within:ring-[#31583b]/10">
            <textarea
